...
The Archives Unleashed Cloud is in active development --- but if you have an Archive-It account, you're welcome to [try it out](http://cloud.archivesunleashed.org)! 
{{< /note >}}
...
From the dashboard, users can select a collection to analyze, which will trigger a number of background jobs. The first job uses data gathered from the WASAPI endpoint to download and verify each ARC/WARC file to our AUK instance. Once the entire collection is downloaded, an automatic email is generated to notify the user that the collection has been downloaded, and analysis will begin. 

The analysis process triggers an Apache Spark job and uses AUT to create a basic set of derivatives:
